...
def basic_dash2(segment_number, bitrates, average_dwn_time,
    ...
    download_rate = DELTA * download_rate + (1 - DELTA) * (recent_download_sizes[-1] * 8 / previous_segment_times[-1])
    ...
    if download_rate > current_bitrate * config_dash.BASIC_UPPER_THRESHOLD:
        ...
        if current_bitrate == bitrates[-1]:
            ...
            try:
                ...
                next_rate = bitrates[current_index + 1] if(download_rate > bitrates[current_index + 1] * config_dash.BASIC_UPPER_THRESHOLD) else current_bitrate
            except ValueError:
